...
import (
	...
	"github.com/virtual-kubelet/nomad"
...
func runRootCommand(ctx context.Context, c Opts) error {
	...
	if c.Provider != "" && c.Provider != "nomad" {
		return errdefs.InvalidInputf("provider %q is not supported: only the nomad provider is supported", c.Provider)
	}

	p, err := nomad.NewProvider(rm, c.NodeName, c.OperatingSystem)
	if err != nil {
		return err
	}
